...
#define sock_timeout_recv 8//设置接收超时
#define sock_timeout_send 8//置发送超时
...
bool set_sock(int sfd_acc);//设置客户端socket
...
bool fork_mission(void){
  struct sockaddr addr;
    ...
    if(_g_srv.pid_pool[tmp] == -1){
    	...
    	//当前新建的子进程, 不停等待客户端的数据到来, 知道超时or 出错
			//设置socket
			bool xend = set_sock(sfd_acc);
			if(xend){//设置socket 正确才能开始io
				xend = do_mission(sfd_acc);
				if(!xend)
...
//设置接收缓冲区
  ...
  opt_val = setsockopt(sfd_acc, SOL_SOCKET, SO_SNDBUF, \
	...
	opt_val = setsockopt(sfd_acc, SOL_SOCKET, SO_RCVTIMEO,\
	...
	opt_val = setsockopt(sfd_acc, SOL_SOCKET, SO_SNDTIMEO,\
							&ptimeout, sizeof(struct timeval));
	if(opt_val == -1){
...
bool do_mission(int sfd_acc){
	...
	//接收一次客户端数据
	int opt_val = recv(sfd_acc, &xbuf, io_buf_max, 0);
	if(opt_val == 0){//对端已经关闭
  ...
  //显示一次客户端数据
	_printf("recv data from client:%s, len = %d\n", xbuf, opt_val);
